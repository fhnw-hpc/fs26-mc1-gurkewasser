...
profiles_dir = Path(__file__).parent / 'profiles/small-test'

baseline = pstats.Stats(f'{profiles_dir}/kafka_producer_baseline_run1.prof')
bottleneck = pstats.Stats(f'{profiles_dir}/kafka_producer_bottleneck_run1.prof')
...
baseline_send = get_func_cumulative(baseline.stats, ['send', 'produce', 'flush'])
baseline_other = baseline_time - baseline_sleep - baseline_json - baseline_send
...
bottleneck_send = get_func_cumulative(bottleneck.stats, ['send', 'produce', 'flush'])
bottleneck_other = bottleneck_time - bottleneck_sleep - bottleneck_json - bottleneck_send

ax2 = plt.subplot(1, 3, 2)
categories = ['time.sleep', 'json.dumps', 'send/produce', 'Other']
baseline_vals = [baseline_sleep, baseline_json, baseline_send, baseline_other]
bottleneck_vals = [bottleneck_sleep, bottleneck_json, bottleneck_send, bottleneck_other]
...
key_funcs = {
    ...
    'send': lambda f: '.send' in f or 'send' in f,
    'produce': lambda f: 'produce' in f,
    'flush': lambda f: 'flush' in f,
}
baseline_dict = {}
bottleneck_dict = {}
